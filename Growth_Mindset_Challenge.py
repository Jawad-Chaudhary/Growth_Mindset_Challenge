...
import pandas as pd
import os # Need to work with the files
from io import BytesIO # To work with the files

#  Set up our app
st.set_page_config(page_title="📑 Data Sweeper", layout="wide")
st.title("Growth_Mindset_Challenge")
st.subheader("📑 Data Sweeper")
st.write("Transfer Your Files Between CSV, Excel format with built-in data cleaning and visualization!")
...
if uploaded_files:
    for file in uploaded_files:
        ...
        if file_ext == ".csv":
            ...
            st.error(f"❌ Unsuppotred File Format: {file_ext}")
            continue 
        
        # Display info about the file
        st.write(f"**File Name:** {file.name}")
        st.write(f"**File Size:** {file.size/1024:.3f} MB")
        
        # Show 5 rows of our data frames
        st.write("🔎 Preview the Head of the Dataframe")
        st.dataframe(df.head())
        
        # Options for data cleaning
        st.subheader("⚙ Data Cleaning Options")
        if st.checkbox(f"Clean Data fot {file.name}"):
